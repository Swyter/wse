...
template <typename V>
class hkStorageStringMap : public hkStringMap<V>
{
	public:
		...
		HK_FORCE_INLINE const char* insert( const char* key, V val )
		{
			...
			if( ! this->isValid(it) )
			{
				ret = hkString::strDup(key);
				hkStringMap<V>::insert( ret, val );
			}
			else
			{
				ret = this->getKey(it);
		...
		void clear()
		{
			for( Iterator it = this->getIterator(); isValid(it); it = getNext(it) )
			{
				char* k = (char*)this->m_map.getKey(it);
				hkDeallocate( k );
			}
		...
		void merge( const hkStringMap<V>& other )
		{
			typedef typename hkStringMap<V>::Iterator OtherIterator;
