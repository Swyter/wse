...
public:
	...
	HK_FORCE_INLINE void setAabbBound(const hkSplitType bound[4]);  
...
private:
	...
	union {
		Leaf		m_leaf;
		Node		m_node;
		hkSplitType	m_bound[4];	// The min or max of an embedded AABB
	} m_data; // 8 Bytes per node 

};

// <ce.todo> Move ProjectedEntry out of BuildInput namespace
...
public:
	...
	HK_FORCE_INLINE hkKdTreeBuildInput::ProjectedEntry* getProjectedEntries() { return m_projectedEntries.begin(); }
	HK_FORCE_INLINE const hkKdTreeBuildInput::ProjectedEntry* getProjectedEntries() const { return m_projectedEntries.begin(); }
...
protected:
	...
	hkArray<struct hkKdTreeBuildInput::ProjectedEntry> m_projectedEntries;
