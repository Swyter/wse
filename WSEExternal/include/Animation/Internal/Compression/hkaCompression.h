...
extern "C"
{
	...
	void HK_CALL hkInverseWaveletTransform( hkReal* waveletData, hkUint32 nValues );
		...
		/// If useTruncation is false, we clamp values such that error is less than the tolerance ie tolerance is a thresholding value.
		/// If useTruncation is true, we clamp a proportion of values given by tolerance, ie. tolerance should be between 0 (lossless) and 1 (fully lossy).
