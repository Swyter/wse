...
HK_ALWAYS_INLINE const hkKdTreeBuildInput::ProjectedEntry* HK_CALL hkGetProjectedEntry(const hkKdTreeNode& spuNode, const hkKdTreeBuildInput::ProjectedEntry* base, int primIdx)
{
	const int aabbIdx = (int) spuNode.getPrimitiveId();
	const hkKdTreeBuildInput::ProjectedEntry* const pe = base + aabbIdx + primIdx;
	return pe;
}
