...
class hkMatrix4
{
	public:
		...
		void resetFourthRow ();
			...
			/// Transforms a position - assumes the the matrix represents an affine transform.
		HK_FORCE_INLINE void transformPosition (const hkVector4& positionIn, hkVector4& positionOut) const;

			/// Transforms a direction - assumes the matrix represents an affine transform.
