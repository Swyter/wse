...
class hkpWorld : public hkReferencedObject
{
	public:
		...
		static void HK_CALL registerWithJobQueue( hkJobQueue* jobQueue );
			...
			/// step.
		void stepMultithreaded( hkJobQueue* jobQueue, hkJobThreadPool* threadPool, hkReal physicsDeltaTime );
		...
		void initMtStep( hkJobQueue* jobQueue, hkReal physicsDeltaTime );
		...
		void finishMtStep( hkJobQueue* jobQueue = HK_NULL, hkJobThreadPool* threadPool = HK_NULL );
