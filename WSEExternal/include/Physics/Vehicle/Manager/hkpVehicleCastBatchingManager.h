...
class hkpVehicleCastBatchingManager : public hkpVehicleManager
{
	public:
		...
		virtual void stepVehiclesSynchronously( hkpWorld* world, hkJobThreadPool* threadPool, hkJobQueue* jobQueue, int numJobs );
		...
		virtual void updateBeforeCollisionDetection();
		...
		virtual int getBufferSize( int numJobs ) = 0;
			...
			/// for a job).
		virtual int buildAndAddCastJobs( const hkpWorld* world, hkInt32 filterSize, int numJobs, hkJobQueue* jobQueue, hkSemaphoreBusyWait* semaphore, void* buffer ) = 0;
		...
		virtual void stepVehiclesUsingCastResults( const hkStepInfo& stepInfo, int numJobs, void* buffer ) = 0;
