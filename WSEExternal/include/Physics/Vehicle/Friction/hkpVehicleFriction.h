...
/// The class hkpVehicleFrictionDescription is used by vehicle dynamics as a const description
/// of how to simulate the friction of a vehicle this class can be initialized using
...
struct hkpVehicleFrictionDescription
{
	...
	struct Cinfo
	{
			...
			/// Note: If in the final game the wheels change position, this internal table can get slightly incorrect.
		...
		hkReal m_chassisMassInv;
			...
			/// This is the inertia of the wheel around the axle
		hkReal m_wheelAxleAngularInertia[2];/*HK_VEHICLE_FRICTION_N_AXIS*/
			/// This is the wheels radius
		hkReal m_wheelRadius[2];/*HK_VEHICLE_FRICTION_N_AXIS*/

			/// Equalizes the friction between front and back wheels: If set to 0, realistic behaviour,
			/// if set to 1.0, the system redistributes the wheel down forces to be the down forces of a 
			/// non moving, in its resting position car. Values between 0 and 1 interpolate between those values
	...
	struct AxisDescription
	{
		...
		hkReal  m_wheelRadiusInv;
			...
			// the final wheeldown force will be calculated using the following formulas:
