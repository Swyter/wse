...
public:
	...
	/// Tests if a bown is considered "down".  Results are OR'd into the result vector
	/// bone Bone index
	/// up Up vector
	...
	static void HK_CALL arrayMaximum( const hkArray< hkReal >& a, const hkArray< hkReal >& b, hkArray< hkReal >& out );

	static void HK_CALL arrayMinimum( const hkArray< hkReal >& a, const hkArray< hkReal >& b, hkArray< hkReal >& out );


	/// Tests if a bown is in front of another bone
	...
	/// \param aniamtion World space animation to analyze
	/// \param val Value
	static int getCyclicEventFrame( const hkArray< hkBool >& values, hkBool val );

	/// \return The index of the first frame with value == val and cyclic previous != val
	static int findFirst( hkBool val, const hkArray< hkBool >& values );

	/// Find frames with value == val and cyclic previous != val
	static void findTransitions( hkBool val, const hkArray< hkBool >& values, hkArray< int >& framesOut );
};
