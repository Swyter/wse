...
class hkpWorldCinfo : public hkReferencedObject
{
	...
	public:
		...
		hkReal m_snapCollisionToConcaveEdgeThreshold; //+default(0.698f)
			...
			/// they will cause a welding artefact.  If you set this to true, Tois will never produce welding artefacts,
		...
		hkReal m_toiCollisionResponseRotateNormal; //+default(0.2f)
			...
			/// The maximum number of hkpAgentSector elements that will be processed by each task split-off from an hkAgentSectorJob.
		int m_maxSectorsPerCollideTask; //+default(4)

			/// If set to true (default), the continuous TOI solver will run collision detection multithreaded (used in hkMultithreadedSimulation only)
			/// If set to false, the old-style single threaded variant will be used. 
			/// TOIs in general have to be solved sequentially and therefore cannot be solved multithreaded.
			/// However the collision detection part of each TOI can be parallelized and solved by different threads.
			/// As a result you should see a TOI solving speedup if a fast complicated object (e.g. listshape) hits a pile of other complicated objects.
			/// However you will not get any speedup (but rather a slow-down) if a simple object (e.g. ragdoll bone) hits another single object (e.g. landscape).
			/// You should try to experiment with that parameter to see which value suits your scenario best.
			/// Note: if you enable this flag the Havok worker threads will not be released until the stepDeltaTime() call is finished,
			///       i.e. you cannot use them for different purposes even if they are idle.
		hkBool m_processToisMultithreaded;	//+default(true)

			/// The maximum number of hkpAgentNnEntry elements that will be processed by each task split-off from an hkAgentNnEntryJob.
		...
		hkBool m_enableSimulationIslands; //+default(true)
			...
			/// however its not done and those islands are flagged with hkpSimulationIsland::m_sparseEnabled = true.
