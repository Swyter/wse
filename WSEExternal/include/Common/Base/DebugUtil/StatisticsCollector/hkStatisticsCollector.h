...
///
/// If an object is serialized in, its memory is is marked as not allocated. As it stands this memory
...
class hkStatisticsCollector: public hkReferencedObject
{
	public:
		...
		virtual void popDir() = 0;
            ...
            /// (Implemented in hkThreadMemory.cpp)
        void addAllocated( const char* name, const void* chunkAddress );
		...
		static int HK_CALL getAllocatedSize(int size);
        ...
        HK_FORCE_INLINE void addArray( const char* name, const hkArray<T>& array )
		{
			...
			{
				int allocated = getAllocatedSize(array.getCapacity() * hkSizeOf(T));
                addChunk( MEM_ARRAY, name, array.begin(), array.getSize() * hkSizeOf(T), allocated );
			}
		}
		template<typename T>
		HK_FORCE_INLINE void addArray( const char* name, const hkObjectArray<T>& array )
		{
		...
		template<typename T>
        HK_FORCE_INLINE void addObjectArray( const char* name, const hkObjectArray<T>& array )
		{
        ...
        HK_FORCE_INLINE void addPointerMap( const char* name, const hkPointerMap<KEY,VALUE>& map )
		{
			...
			int allocated = getAllocatedSize(used);
		...
		static void HK_CALL memorySizeToText(int size, hkString& stringOut);
