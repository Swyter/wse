...
class hkpCharacterProxyListener
{
	public:
		...
		virtual void processConstraintsCallback( const hkArray<hkpRootCdPoint>& manifold, hkpSimplexSolverInput& input ) {}

			/// Called when a new contact point is taken from the results of the linear cast and added to the current manifold
		virtual void contactPointAddedCallback(const hkpRootCdPoint& point) {}

			/// Called when a new contact point is discarded from the current manifold
		virtual void contactPointRemovedCallback(const hkpRootCdPoint& point) {}
