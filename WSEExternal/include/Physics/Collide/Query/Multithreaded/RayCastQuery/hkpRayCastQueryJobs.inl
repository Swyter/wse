...
hkpKdTreeRayCastJob::hkpKdTreeRayCastJob(hkpRayCastQueryJobHeader* jobHeader, const hkKdTreeRayCastCommand* commandArray, int numCommands, hkSemaphoreBusyWait* semaphore, int numCommandsPerTask) 
: 	hkpRayCastQueryJob(RAYCAST_QUERY_KD_TREE_RAYCAST, sizeof(hkpKdTreeRayCastJob) )
{
	HK_ASSERT2( 0xaf236152, jobHeader && ((hkUlong)jobHeader & 0xf) == 0,											"jobHeader has to be set and be 16 byte aligned." );
	HK_ASSERT2( 0xaf3647f6, numCommands > 0,																		"numCommands has to be > 0." );
	HK_ASSERT2( 0xaf3647f7, commandArray && ((hkUlong)commandArray & 0xf) == 0,										"commandArray has to be set and 16 byte aligned." );
...
#if defined(HK_DEBUG)
	{
		hkKdTreeCheckCommandPointers<hkKdTreeRayCastCommand>(commandArray, numCommands);
		hkKdTreeCheckCommandCapacity<hkKdTreeRayCastCommand>(commandArray, numCommands);
	}
#endif
	...
	m_numCommandsPerTask	= numCommandsPerTask;
	m_semaphore				= semaphore;
	m_rayCastCommandArray			= commandArray;
	m_numRayCastCommands			= numCommands;

	m_rayCastBundleCommandArray			= 0;
	m_numRayCastBundleCommands			= HK_NULL;
...
hkJobQueue::JobPopFuncResult hkpKdTreeRayCastJob::popJobTask( hkpKdTreeRayCastJob& out )
{
	//
	// split off a fully filled child job if there are more commands left than one job can handle
	//

	hkJobQueue::JobPopFuncResult retVal =  hkJobQueue::POP_QUEUE_ENTRY;
	if ( m_numRayCastCommands > m_numCommandsPerTask )
	{
		out.m_numRayCastCommands	 = m_numCommandsPerTask;
		m_numRayCastCommands		-= m_numCommandsPerTask;
		m_rayCastCommandArray		 = hkAddByteOffsetConst( m_rayCastCommandArray, m_numCommandsPerTask * sizeof(hkKdTreeRayCastCommand) );

		retVal = hkJobQueue::DO_NOT_POP_QUEUE_ENTRY;
	}

	const int numBundleCommandsPerTask = m_numCommandsPerTask/4;
	if ( m_numRayCastBundleCommands > numBundleCommandsPerTask )
	{
		out.m_numRayCastBundleCommands	 = numBundleCommandsPerTask;
		m_numRayCastBundleCommands		-=  numBundleCommandsPerTask;
		m_rayCastBundleCommandArray		 = hkAddByteOffsetConst( m_rayCastBundleCommandArray, numBundleCommandsPerTask * sizeof(hkKdTreeRayCastBundleCommand) );
