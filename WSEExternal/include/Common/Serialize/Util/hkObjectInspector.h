...
namespace hkObjectInspector
{
	...
	class ObjectListener
	{
		public:
			...
			virtual hkResult objectCallback( const void* objP, const hkClass& klass, hkArray<Pointer>& containedPointers ) = 0;
	};
		...
		/// for virtual object. However, in some cases where the base
		/// class for virtual object is unknown it contains HK_NULL.
	hkResult HK_CALL getPointers(const void* object, const hkClass& klass, hkArray<Pointer>& outContainedPointers);
