...
{
	HK_ASSERT( 0x0, (offset & 0x3) == 0);
	HK_ASSERT( 0x0, (offset & 0x80000000) == 0);
...
void hkKdTreeNode::setAabbBound(const hkSplitType bound[4])
{
	m_data.m_bound[0] = bound[0];
	m_data.m_bound[1] = bound[1];
	m_data.m_bound[2] = bound[2];
	m_data.m_bound[3] = bound[3];
}
