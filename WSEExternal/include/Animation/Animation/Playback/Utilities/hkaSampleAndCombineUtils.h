...
class hkaSampleAndCombineUtils
{
	...
	struct FloatBlendParameters
	{
		...
		/// where, for each bone 'i' in the pose: <br>
		/// P = accumulationPoseInOut[i] <br>
		/// Q = paramsIn.m_animationTransformsIn[i] <br>
		/// W = accumulatedPerBoneWeightsInOut[i] <br>
		/// t = paramsIn.m_perBoneWeights[i]/255.0f (or 1.0 if there are no per bone weights in the control) <br>
		/// P' = accumulationPoseInOut[i] after above modification <br>
		/// W' = accumulatedPerBoneWeightsInOut[i] after above modification <br>
		/// The final result is not a normalized pose, and should be scaled by the wieghts array W' before use.
