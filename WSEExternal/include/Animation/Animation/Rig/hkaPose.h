...
#include <Common/Base/Container/LocalArray/hkLocalArray.h>
	...
	/// Both representations are kept in sync lazily, using the hkaSkeleton hierachy.
	/// For the float data it simply has an array of 'float slot values' into which the float data can be sampled - the skeleton hierachy has no effect here.
	...
	/// pointers to reserved blocks of data for the sampling to write into) but this class facilitates model/local space syncing via the hkaSkelton
	/// associated with it.
...
public:
	...
	*/
	...
	HK_FORCE_INLINE void setFloatSlotValues (const hkArray<hkReal>& floatSlotValues);
