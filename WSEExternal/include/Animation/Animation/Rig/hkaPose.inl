...
int hkaPose::getRequiredMemorySize (const hkaSkeleton* skeleton)
{
	...
	const int totalSize =  numBones * ( 2 * hkSizeOf(hkQsTransform) + hkSizeOf(hkaPoseFlag) )
...
hkaPose::hkaPose(const hkaSkeleton* skeleton, void* preallocatedMemory)
	:	m_skeleton  (skeleton), 
		...
		m_boneFlags (reinterpret_cast<hkaPoseFlag*> (m_modelPose.begin() + skeleton->m_numBones), skeleton->m_numBones, skeleton->m_numBones),
		m_modelInSync (false),
		m_localInSync (false),
		m_floatSlotValues (reinterpret_cast<hkReal*> (m_boneFlags.begin() + skeleton->m_numBones), skeleton->m_numFloatSlots, HK_NEXT_MULTIPLE_OF( 4, skeleton->m_numFloatSlots))
{
...
void hkaPose::setFloatSlotValues(const hkArray<hkReal>& floatSlotValues)
{
