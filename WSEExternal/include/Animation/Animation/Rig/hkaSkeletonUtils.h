...
class hkaSkeletonUtils
{
	public:
		...
		static void HK_CALL lockTranslations (const hkaSkeleton& skeleton, hkBool exceptRoots = true);

			/// Unlocks all the translations for the given skeleton
		static void HK_CALL unlockTranslations (const hkaSkeleton& skeleton);

			/// Fills bonesOut with the chain between start bone and end bone (where end bone is deeper in the hierarchy)
			/// Returns false if there is no possible chain - NOTE :L The array will be modified anyway
