...
public:
	...
	static struct hkpWorldRayCastOutputPpu* HK_CALL getNextFreeResult(struct hkKdTreeRayCastCommand* command, struct hkpWorldRayCastOutputPpu* resultsArray, HK_PAD_ON_SPU(hkpWorldRayCastOutputPpu*)& nextFreeResult, hkReal hitFraction, hkReal& maxHitFractionOut);

	static void HK_CALL initializeBuildSetupJobs(const hkpCollidable** collidablesPtr, int numCollidables, hkKdTreeBuildInput::ProjectedEntry* entries, hkSemaphoreBusyWait* semaphore, int numJobs, hkpKdTreeBuildSetupJob* jobsOut);
};
