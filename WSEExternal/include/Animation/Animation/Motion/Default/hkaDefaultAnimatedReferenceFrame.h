...
class hkaDefaultAnimatedReferenceFrame : public hkaAnimatedReferenceFrame
{
	public:
		...
		hkaDefaultAnimatedReferenceFrame( const MotionExtractionOptions& options );
			...
			/// Destructor; it deallocates the memory used to store the motion track
		...
		virtual void getDeltaReferenceFrame( hkReal time, hkReal nextTime, int loops, hkQsTransform& output ) const;
	...
	public:
		...
		hkVector4* m_referenceFrameSamples;
		/// Size of m_referenceFrameSamples array.
		int m_numReferenceFrameSamples;
	...
	public:
		...
		HK_FORCE_INLINE hkaDefaultAnimatedReferenceFrame( hkFinishLoadedObjectFlag flag ) : hkaAnimatedReferenceFrame(flag) {}
